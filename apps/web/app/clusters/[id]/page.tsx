...
export default function ClusterPage({params}:{params:Promise<{id:string}>}){
  ...
  useEffect(()=>{params.then(async p=>{setId(p.id);try{const loaded=await getCluster(p.id);setCluster(loaded);const [memory,featureRead]=await Promise.all([related(p.id),getFeature(loaded.feature_id)]);setMemories(memory);setFeature(featureRead);}catch(e){setError(e instanceof Error?e.message:"Unavailable")}})},[params]);
