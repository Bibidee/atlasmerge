...
export default function ClusterPage({params}:{params:Promise<{id:string}>}){
  ...
  async function refresh(clusterId:string){const loaded=await getCluster(clusterId);const [memory,featureRead]=await Promise.all([related(loaded.feature_id).catch(()=>[]),getFeature(loaded.feature_id)]);setCluster(loaded);setMemories(memory);setFeature(featureRead);return loaded;}
